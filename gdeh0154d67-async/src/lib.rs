    ...
    Delay: DelayUs,
{
    ...
    pub async fn initialize(&mut self) -> Result<(), Error<E>> {
        ...
        self.set_ram_x_start_end_position(0, 200)?;
        self.set_ram_y_start_end_position(0, 200)?;
        self.set_border_waveform()?;
        self.set_temperature_sensor(TemperatureSensor::Internal)?;
        self.set_display_update_sequence(0xb1)?;
    ...
    fn set_driver_output(&mut self) -> Result<(), Error<E>> {
        self.write_command_data(command::DRIVER_OUTPUT_CONTROL, &[0xc7, 0b0, 0x00])?;
        Ok(())
    }

    fn set_data_entry_mode(&mut self) -> Result<(), Error<E>> {
        self.write_command_data(command::DATA_ENTRY_MODE_SETTING, &[0b0_11])?;
        Ok(())
    }

    fn set_ram_x_start_end_position(&mut self, x: u16, width: u16) -> Result<(), Error<E>> {
    ...
    fn set_ram_y_start_end_position(&mut self, y: u16, height: u16) -> Result<(), Error<E>> {
        self.write_command_data(
    ...
    fn set_ram_x_address_position(&mut self, x: u16) -> Result<(), Error<E>> {
        self.write_command_data(command::SET_RAM_X_ADDRESS_POSITION, &[(x / 8) as u8])?;
        Ok(())
    }

    fn set_ram_y_address_position(&mut self, y: u16) -> Result<(), Error<E>> {
        self.write_command_data(
    ...
    fn set_border_waveform(&mut self) -> Result<(), Error<E>> {
        self.write_command_data(command::BORDER_WAVEFORM_CONTROL, &[0b101])?;
        Ok(())
    }

    fn set_temperature_sensor(&mut self, sensor: TemperatureSensor) -> Result<(), Error<E>> {
    ...
    fn set_display_update_sequence(&mut self, sequence: u8) -> Result<(), Error<E>> {
        self.write_command_data(command::DISPLAY_UPDATE_CONTROL_2, &[sequence])?;
        Ok(())
    }

    async fn master_activation(&mut self) -> Result<(), Error<E>> {
        self.write_command(command::MASTER_ACTIVATION)?;
        self.busy_wait().await;
        Ok(())
    }

    fn write_bw_ram(&mut self, data: &[u8]) -> Result<(), Error<E>> {
        self.write_command_data(command::WRITE_RAM_BW, data)?;
        Ok(())
    }

    fn set_deep_sleep_mode(&mut self, mode: DeepSleepMode) -> Result<(), Error<E>> {
    ...
    fn write_command_data(&mut self, command: u8, data: &[u8]) -> Result<(), Error<E>> {
        self.write_command(command)?;
        self.write_data(data)?;
        Ok(())
    }

    fn write_command(&mut self, command: u8) -> Result<(), Error<E>> {
        self.dc.set_low().unwrap();
